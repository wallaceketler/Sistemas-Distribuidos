...
public class PlayerO extends javax.swing.JFrame {
    ...
    public PlayerO() {
        ...
        new Thread(()->{
            ...
            try {   
                ...
                ServerSocket reading_playerO_controller = new ServerSocket(6060);
                String controller_data;
                //para receber turno inicial do controller
                ServerSocket read_initial_turn = new ServerSocket(8080);
                Socket send_initial_turn_toO = read_initial_turn.accept();
                ObjectInputStream data_initial_turn = new ObjectInputStream(send_initial_turn_toO.getInputStream());
                ...
                while(true){
                    ...
                    Socket controller_sending_playerO = reading_playerO_controller.accept();
                    ObjectInputStream data_receiving_controller  = new ObjectInputStream(controller_sending_playerO.getInputStream());
                ...
                JOptionPane.showMessageDialog(null,"Erro player O:" + e);
            }}
        ).start();
    ...
    private void centerMidActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_centerMidActionPerformed
        if(turn.equals("O")){
            new Thread(()->{
               try{
                   Socket playerO_sending = new Socket("127.0.0.1", 4040);
                   ObjectOutputStream playerO_data = new ObjectOutputStream(playerO_sending.getOutputStream());
                    playerO_data.flush();
                    playerO_data.writeUTF("centerMid-O");

                   //Finaliza ObjectOutputStream e Socket
                   playerO_data.close();
                   playerO_sending.close();
               }catch(Exception e){
                   JOptionPane.showMessageDialog(null,"Erro Botão do PlayerO: " + e);
               }
           }).start();
        }
        ...
        if(turn.equals("O")){
            new Thread(()->{
               try{
                   ...
                   ObjectOutputStream playerO_data = new ObjectOutputStream(playerO_sending.getOutputStream());
                    playerO_data.flush();
                    playerO_data.writeUTF("centerRight-O");
        ...
        if(turn.equals("O")){
            new Thread(()->{
               try{
                   ...
                   ObjectOutputStream playerO_data = new ObjectOutputStream(playerO_sending.getOutputStream());
                    playerO_data.flush();
                    playerO_data.writeUTF("upLeft-O");
        ...
        if(turn.equals("O")){
            new Thread(()->{
               try{
                   ...
                   ObjectOutputStream playerO_data = new ObjectOutputStream(playerO_sending.getOutputStream());
                    playerO_data.flush();
                    playerO_data.writeUTF("upRight-O");
        ...
        if(turn.equals("O")){
            new Thread(()->{
               try{
                   ...
                   ObjectOutputStream playerO_data = new ObjectOutputStream(playerO_sending.getOutputStream());
                    playerO_data.flush();
                    playerO_data.writeUTF("centerLeft-O");
        ...
        if(turn.equals("O")){
            new Thread(()->{
               try{
                   ...
                   ObjectOutputStream playerO_data = new ObjectOutputStream(playerO_sending.getOutputStream());
                    playerO_data.flush();
                    playerO_data.writeUTF("downLeft-O");
        ...
        if(turn.equals("O")){
            new Thread(()->{
               try{
                   ...
                   ObjectOutputStream playerO_data = new ObjectOutputStream(playerO_sending.getOutputStream());
                    playerO_data.flush();
                    playerO_data.writeUTF("downMid-O");
        ...
        if(turn.equals("O")){
            new Thread(()->{
               try{
                   ...
                   ObjectOutputStream playerO_data = new ObjectOutputStream(playerO_sending.getOutputStream());
                    playerO_data.flush();
                    playerO_data.writeUTF("downRight-O");
